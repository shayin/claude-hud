...
interface MiniMaxUsageApiResponse {
  model_remains?: Array<{
    start_time?: number;
    end_time?: number;
    remains_time?: number;
...
interface MiniMaxUsageApiResult {
  data: MiniMaxUsageApiResponse | null;
  error?: string;
}
 ...
 */
function isUsingMiniMaxEndpoint(env: NodeJS.ProcessEnv = process.env): boolean {
  const baseUrl = env.ANTHROPIC_BASE_URL?.trim() || env.ANTHROPIC_API_BASE_URL?.trim();
  if (!baseUrl) {
    return false;
  }
 ...
 */
function fetchMiniMaxUsageApi(env: NodeJS.ProcessEnv = process.env): Promise<MiniMaxUsageApiResult> {
  return new Promise((resolve) => {
    const authToken = env.ANTHROPIC_AUTH_TOKEN?.trim();
    if (!authToken) {
      resolve({ data: null, error: 'no-auth-token' });
 ...
 */
function parseMiniMaxUsageData(response: MiniMaxUsageApiResponse, _env: NodeJS.ProcessEnv = process.env): UsageData {
