 ...
 */
function isGlmEndpoint(baseUrl) {
 ...
 */
function isUsingGlmEndpoint(env = process.env) {
    const baseUrl = env.ANTHROPIC_BASE_URL?.trim() || env.ANTHROPIC_API_BASE_URL?.trim();
    if (!baseUrl) {
        return false;
    }
 ...
 */
function getGlmBaseDomain(env = process.env) {
    const baseUrl = env.ANTHROPIC_BASE_URL?.trim() || env.ANTHROPIC_API_BASE_URL?.trim();
 ...
 */
function fetchGlmUsageApi(env = process.env) {
 ...
 */
async function getGlmUsage(homeDir, now, ttls) {
